...
SineWaves.prototype.update = function(time) {
  ...
  while (++index < length) {
    ...
    this.drawWave(time * timeModifier, this.waves[index], index);
  }
  this.ctx.restore();
 ...
 * @param  {Object} options wave options
 */
SineWaves.prototype.drawWave = function(time, options, index) {
  // Setup defaults
  options = Utilities.defaults(defaultWave, options);
  var optionTranslate = options.translateY;
  // Styles
  ...
  // Starting Line
  this.ctx.moveTo(0, this.yAxis + options.translateY);
  this.ctx.lineTo(this.waveLeft, this.yAxis + options.translateY);
  ...
  for (i = 0; i < this.waveWidth; i += options.segmentLength) {
    ...
    // Draw to it
    this.ctx.lineTo(point.x, point.y + options.translateY);
  ...
  // Ending Line
  this.ctx.lineTo(this.width, this.yAxis + optionTranslate);
